...
async fn run_server(args: &CliArgs) -> Result<()> {
    ...
    start_poller_loop(&client, &args.targets, &args.state_dir, poll_interval).await?;
...
async fn start_poller_loop(
    ...
    let mut states: Vec<(Target, Box<dyn State>)> = targets
        .iter()
        .map(|t| t.load_state(state_dir).map(|s| (*t, s)))
        .collect::<Result<_>>()?;
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                ...
                for (target, state) in &mut states {
                    if let Err(e) = state.poll(client).await {
                        error!(target = ?target, "Poll failed: {e:#}");
                        any_failure = true;
                        continue;
                    }

                    if let Err(e) = target.save_state(state.as_ref(), state_dir) {
                        warn!(target = ?target, "Failed to save state. Metrics may be incorrect after program restart: {e:#}");
                        any_failure = true;
                    }

                    gauge!(format!("bunnynet.{}.last_update.timestamp_seconds", target.name())).set(now());
                }
